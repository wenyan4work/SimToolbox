...
void ConstraintSolver::setup(const double dt) {
    ...
    velExternalRcp_ = Teuchos::rcp(new TV(mobMapRcp_, true));
    forceExternalRcp_ = Teuchos::rcp(new TV(mobMapRcp_, true));
    ptcSystemPtr_->getForceVelocityNonConstraint(forceExternalRcp_, velExternalRcp_);
}
...
void ConstraintSolver::reset() {
    ...
    velConRcp_.reset();
    forceConRcp_.reset();
    velExternalRcp_.reset();
    forceExternalRcp_.reset();
}
...
void ConstraintSolver::solveConstraints() {
    ...
    ///////////////////////////////////////
    Teuchos::RCP<EvaluatorTpetraConstraint> model = Teuchos::rcp(new EvaluatorTpetraConstraint(commRcp_, mobOpRcp_, velExternalRcp_, 
                                                        forceExternalRcp_, conCollectorPtr_, ptcSystemPtr_, forceConRcp_, velConRcp_, dt_));
