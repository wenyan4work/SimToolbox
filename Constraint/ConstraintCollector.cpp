...
int ConstraintCollector::buildConstraintMatrixVector(
  ...
  const auto &cQueOffset = buildConQueOffset();
...
#pragma omp parallel for num_threads(cPoolSize)
  ...
  for (auto &v : colIndexOffset) {
    printf("colIndexOffset %d\n", v);
  }
...
void ConstraintCollector::writeConstraintBlockPool(const std::string &filename,
  ...
  msgpack::pack(ofs, *constraintPoolPtr);
  return;
}