...
class Particle {
  public:
     ...
     */
    Particle(const int &gid_, const double &radius_, const double &radiusCollision_, const double &length_,
    ...
    /**
     * @brief return if this particle is treated as a sphere
     *
     * @param collision
     * @return true
     * @return false
    ...
    static void writeVTP(const Container &particle, const int particleNumber, const std::string &prefix,
        ...
        // growth stuff that needs renamed
        std::vector<float> t(particleNumber);
        std::vector<float> tg(particleNumber); 
        std::vector<float> tauD(particleNumber);
        std::vector<float> sigma(particleNumber);
        std::vector<float> deltaL(particleNumber);
        std::vector<float> normalStress(particleNumber);

        // vel
        std::vector<float> vel(3 * particleNumber);
        std::vector<float> omega(3 * particleNumber);
        std::vector<float> velCon(3 * particleNumber);
        std::vector<float> omegaCon(3 * particleNumber);
        ...
        std::vector<float> forceCon(3 * particleNumber);
        std::vector<float> torqueCon(3 * particleNumber);
...
#pragma omp parallel for
        for (int i = 0; i < particleNumber; i++) {
            const auto &ptc = particle[i];
            // point and point data
            // store current position and orientation
            Evec3 direction = ECmapq(ptc.orientation) * Evec3(0, 0, 1);
            Evec3 end0 = ECmap3(ptc.pos) - direction * (ptc.length * 0.5);
            Evec3 end1 = ECmap3(ptc.pos) + direction * (ptc.length * 0.5);



            // Evec3 direction = ECmapq(ptc.orientationCurrent) * Evec3(0, 0, 1);
            // Evec3 end0 = ECmap3(ptc.posCurrent) - direction * (ptc.length * 0.5);
            // Evec3 end1 = ECmap3(ptc.posCurrent) + direction * (ptc.length * 0.5);
            pos[6 * i] = end0[0];
            pos[6 * i + 1] = end0[1];
            pos[6 * i + 2] = end0[2];
            pos[6 * i + 3] = end1[0];
            ...
            gid[i] = ptc.gid;
            group[i] = ptc.group;
            isImmovable[i] = ptc.isImmovable ? 1 : 0;

            radius[i] = ptc.radius;
            radiusCollision[i] = ptc.radiusCollision;
            length[i] = ptc.length;
            lengthCollision[i] = ptc.lengthCollision;
            ...
            sigma[i] = ptc.sigma;
            deltaL[i] = ptc.deltaL;
            normalStress[i] = ptc.normalStress;

            // force velocity data
            Evec3 nx = ECmapq(ptc.orientation) * Evec3(1, 0, 0);
            Evec3 nz = ECmapq(ptc.orientation) * Evec3(0, 0, 1);
            for (int j = 0; j < 3; j++) {
                vel[3 * i + j] = ptc.vel[j];
                omega[3 * i + j] = ptc.omega[j];
                velCon[3 * i + j] = ptc.velCon[j];
                omegaCon[3 * i + j] = ptc.omegaCon[j];
                velNonB[3 * i + j] = ptc.velNonB[j];
                omegaNonB[3 * i + j] = ptc.omegaNonB[j];

                force[3 * i + j] = ptc.force[j];
                torque[3 * i + j] = ptc.torque[j];
                forceCon[3 * i + j] = ptc.forceCon[j];
                torqueCon[3 * i + j] = ptc.torqueCon[j];
                forceNonB[3 * i + j] = ptc.forceNonB[j];
                torqueNonB[3 * i + j] = ptc.torqueNonB[j];

                velBrown[3 * i + j] = ptc.velBrown[j];
                omegaBrown[3 * i + j] = ptc.omegaBrown[j];
