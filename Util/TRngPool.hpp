...
private:
  ...
  std::vector<std::unique_ptr<myEngineType>>
      rngEngineThreadsPtr; /// one rng for each local thread

  trng::uniform01_dist<double> u01; ///< \f$U(0,1)\f$ transformer
  trng::normal_dist<double> n01;    ///< \f$N(0,1)\f$ transformer

  std::unique_ptr<trng::lognormal_dist<double>>
      lnDistPtr; ///< \f$logNormal(\mu,\sigma)\f$ transformer
...
#pragma omp parallel for num_threads(nThreads)
   ...
   * @return double
   */
  inline double getU01(int threadId) {
    return u01(*rngEngineThreadsPtr[threadId]);
  }
  inline double getN01(int threadId) {
    return n01(*rngEngineThreadsPtr[threadId]);
  }
  inline double getLN(int threadId) {
    return (*lnDistPtr)(*rngEngineThreadsPtr[threadId]);
  }
