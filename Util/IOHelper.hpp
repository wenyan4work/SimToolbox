...
public:
  ...
  struct FieldVTU {
    ...
    FieldVTU(int dimension_, IOTYPE type_, std::string name_)
        : dimension(dimension_), type(type_), name(name_) {}
  };
  ...
  static void makeSubFolder(const std::string folder) {
    ...
    const int err =
        mkdir(folder.c_str(),
              0755); // make one folder at a time. parent folder must exist
    if (errno == EEXIST) {
  ...
  static void writeHeadVTU(std::ofstream &vtkfile) {
    ...
    vtkfile << "<VTKFile type=\"UnstructuredGrid\" version=\"1.0\" "
  ...
  static void writePVTUFile(std::string filename,
                            const std::vector<FieldVTU> &dataFields,
                            const std::vector<std::string> &pieceNames) {

    std::ofstream pvtufile(filename, std::ios::out);

    pvtufile << "<?xml version=\"1.0\"?>\n";
    pvtufile << "<VTKFile type=\"PUnstructuredGrid\" version=\"1.0\" "
                "byte_order=\"LittleEndian\" "
                "header_type=\"UInt32\"> \n";
    ...
    for (long i = 0; i < dataFields.size(); i++) {
      auto &data = dataFields[i];
      // data.first = dimension
      // data.second = name
      std::string type = getTypeName(data.type);
      pvtufile << "<PDataArray Name=\"" << data.name << "\" type=\"" << type
               << "\" NumberOfComponents=\"" << data.dimension
               << "\" format=\"binary\"/>\n";
    }
    ...
    pvtufile << "<PPoints> \n";
    pvtufile << "<PDataArray NumberOfComponents=\"3\" type=\"Float64\" "
                "format=\"binary\"/>\n";
    pvtufile << "</PPoints> \n";
  ...
  static void writePVTUFile(std::string filename,
                            const std::vector<FieldVTU> &pointDataFields,
                            const std::vector<FieldVTU> &cellDataFields,
                            const std::vector<std::string> &pieceNames) {
    ...
    // point data
    pvtufile << "<PPointData Scalars=\"scalars\">\n";
    for (long i = 0; i < pointDataFields.size(); i++) {
    ...
    for (long i = 0; i < cellDataFields.size(); i++) {
  ...
  static void writeHeadVTP(std::ofstream &vtkfile) {
    vtkfile << "<?xml version=\"1.0\"?>\n";
    vtkfile << "<VTKFile type=\"PolyData\" version=\"1.0\" "
               "byte_order=\"LittleEndian\"  "
               "header_type=\"UInt32\">\n";
    vtkfile << "<PolyData>\n";
  }
  ...
  static void writePVTPFile(std::string filename,
                            const std::vector<FieldVTU> &dataFields,
                            const std::vector<std::string> &pieceNames) {
    ...
    pvtufile << "<?xml version=\"1.0\"?>\n";
    pvtufile << "<VTKFile type=\"PPolyData\" version=\"1.0\" "
                "byte_order=\"LittleEndian\" "
                "header_type=\"UInt32\"> \n";
   ...
   */
  template <class T>
  static void writeDataArrayBase64(std::vector<T> &data,
                                   const std::string &name, int numComp,
                                   std::ofstream &file) {
    // set type name
    std::string vtktype;
    ...
    file << "<DataArray Name=\"" << name << "\" type=\"" << vtktype
         << "\" NumberOfComponents=\"" << numComp << "\" format=\"binary\">\n";
    file << contentB64 << "\n";
    file << "</DataArray>\n";
  }
   ...
   ********************************/
  static void writeHeadVTR(std::ofstream &vtkfile, int boxLow[3],
                           int boxHigh[3]) {
    vtkfile << "<?xml version=\"1.0\"?>\n";
    vtkfile << "<VTKFile type=\"RectilinearGrid\" version=\"1.0\" "
               "byte_order=\"LittleEndian\"  "
               "header_type=\"UInt32\">\n";
    vtkfile << "<RectilinearGrid WholeExtent=\"" << boxLow[0] << " "
            << boxHigh[0] << " " << boxLow[1] << " " << boxHigh[1] << " "
            << boxLow[2] << " " << boxHigh[2] << "\">\n";
  }
   ...
   ********************************/
  static void writeHeadVTI(std::ofstream &vtkfile, int boxLow[3],
                           int boxHigh[3], double spacing[3],
                           double origin[3]) {
    vtkfile << "<?xml version=\"1.0\"?>\n";
    vtkfile << "<VTKFile type=\"ImageData\" version=\"1.0\" "
               "byte_order=\"LittleEndian\"  "
               "header_type=\"UInt32\">\n";
    vtkfile << "<ImageData WholeExtent=\"" << boxLow[0] << " " << boxHigh[0]
            << " " << boxLow[1] << " " << boxHigh[1] << " " << boxLow[2] << " "
            << boxHigh[2] << "\"" //
