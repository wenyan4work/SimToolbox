...
void fixedPoints(int nPts, double box, double shift,
...
void shiftAndScalePoints(std::vector<double> &ptsCoord, double shift[3],
                         double scale) {
...
void dumpPoints(const std::string &filename, std::vector<double> &coordLocal,
  ...
  for (long i = 0; i < npts; i++) {
    fprintf(fp, "%6f %6f %6f;", coord[3 * i], coord[3 * i + 1],
            coord[3 * i + 2]);
    for (long j = 0; j < valueDimension; j++) {
...
void checkError(const std::vector<double> &valueLocal,
                const std::vector<double> &valueTrueLocal, const double bar) {
  ...
  if (myRank == 0) {
    ...
    for (long i = 0; i < valueTrue.size(); i++) {
    ...
    double errorRMS = sqrt(errorL2 / value.size());
    double errorRelL2 = sqrt(errorL2 / L2);
    ...
    if (bar > 0) {
      bool pass = true;
      pass = pass && (errorMaxL2 < bar);
      pass = pass && (errorMaxRel < bar);
      pass = pass && (errorAbs < bar);
      pass = pass && (errorRMS < bar);
      pass = pass && (errorRelL2 < bar);
      if (pass) {
        printf("Test Pass\n");
      } else {
        printf("Error larger than given bar\n");
      }
    }
  }
...
void collectPts(std::vector<double> &pts) {
  ...
  if (myRank == 0) {
    MPI_Gatherv(pts.data(), pts.size(), MPI_DOUBLE, ptsRecv.data(),
                recvSize.data(), displs.data(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
  } else {
