...
class B64Converter {
  static const unsigned char
      kwsysBase64EncodeTable[65]; ///< lookup encoding table

  static const unsigned char
      kwsysBase64DecodeTable[256]; ///< lookup decoding table

  static unsigned char kwsysBase64EncodeChar(long c) {
   ...
   * @param dest
   */
  static void kwsysBase64_Encode3(const unsigned char *src,
                                  unsigned char *dest) {
   ...
   * @param dest
   */
  static void kwsysBase64_Encode2(const unsigned char *src,
                                  unsigned char *dest) {
    dest[0] = kwsysBase64EncodeChar((src[0] >> 2) & 0x3F);
    dest[1] =
        kwsysBase64EncodeChar(((src[0] << 4) & 0x30) | ((src[1] >> 4) & 0x0F));
   ...
   * @param dest
   */
  static void kwsysBase64_Encode1(const unsigned char *src,
                                  unsigned char *dest) {
    dest[0] = kwsysBase64EncodeChar((src[0] >> 2) & 0x3F);
   ...
   * @param dest
   */
  static int kwsysBase64_Decode3(const unsigned char *src,
                                 unsigned char *dest) {
  ...
  /* Encode 'length' bytes from the input buffer and store the
     ...
     buffer (output). Note that the output buffer must be allocated by the
     caller (length * 1.5 should be a safe estimate).  If 'mark_end' is true
     than an extra set of 4 bytes is added to the end of the stream if the input
     is a multiple of 3 bytes.  These bytes are invalid chars and therefore they
     will stop the decoder thus enabling the caller to decode a stream without
...
public:
   ...
   */
  static size_t kwsysBase64_Encode(const unsigned char *input, size_t length,
                                   std::string &output, bool mark_end = false) {
    // append to output
    // VTK does not allow '\n' in the encoded data
  ...
  static size_t kwsysBase64_Decode(const unsigned char *input, size_t length,
   ...
   */
  template <class T>
  static void getBase64FromVector(const std::vector<T> &vec,
                                  std::string &result) {
    ...
    const uint32_t length =
        vec.size() *
        (sizeof(T) /
         sizeof(unsigned char)); // length of actual bytes, not just vec.size()
    // encode the total length
    kwsysBase64_Encode(reinterpret_cast<const unsigned char *>(&length), 4,
                       result);
    // encode the actual data
    kwsysBase64_Encode(reinterpret_cast<const unsigned char *>(vec.data()),
                       (sizeof(T) / sizeof(unsigned char)) * vec.size(),
                       result);
  }
};
