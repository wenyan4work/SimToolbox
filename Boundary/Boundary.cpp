...
void SphereShell::project(const double query[3], double project[3],
  ...
  Evec3 Proj =
      radius * (1 / QueryR) * Query; // radius * norm direction of Query
  Evec3 PQ = Query - Proj;
...
bool SphereShell::check(const double query[3], const double project[3],
                        const double delta_[3]) const {
...
void Wall::project(const double query[3], double project[3],
                   double delta[3]) const {
  Evec3 Query = ECmap3(query);
...
bool Wall::check(const double query[3], const double project[3],
                 const double delta_[3]) const {
  std::cout << "query: " << ECmap3(query).transpose() << "\t";
  std::cout << "project: " << ECmap3(project).transpose() << "\t";
  std::cout << "delta: " << ECmap3(delta_).transpose() << std::endl;
...
void Tube::project(const double query[3], double project[3],
                   double delta_[3]) const {
  Evec3 Query = ECmap3(query);
...
bool Tube::check(const double query[3], const double project[3],
                 const double delta_[3]) const {
  std::cout << "query: " << ECmap3(query).transpose() << "\t";
  std::cout << "project: " << ECmap3(project).transpose() << "\t";
  std::cout << "delta: " << ECmap3(delta_).transpose() << std::endl;
