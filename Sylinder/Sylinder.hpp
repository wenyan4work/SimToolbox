...
public:
  ...
  static void writePVTP(const std::string &prefix, const std::string &postfix,
    ...
    cellDataFields.emplace_back(1, IOHelper::IOTYPE::Float32,
                                "radiusCollision");
    ...
    cellDataFields.emplace_back(3, IOHelper::IOTYPE::Float32,
                                "torqueCollision");
    ...
    for (long i = 0; i < nProcs; i++) {
      pieceNames.emplace_back(std::string("Sylinder_") + std::string("r") +
                              std::to_string(i) + "_" + postfix + ".vtp");
    }

    IOHelper::writePVTPFile(prefix + "Sylinder_" + postfix + ".pvtp",
   ...
   * Rank 0 writes the parallel header , then each rank write its own serial
   * vtp/vtu file
   *
  ...
  static void writeVTP(const Container &sylinder, const int sylinderNumber,
...
#pragma omp parallel for
    ...
    std::ofstream file(prefix + std::string("Sylinder_") + "r" +
    ...
    file << "<Piece NumberOfPoints=\"" << sylinderNumber * 2
         << "\" NumberOfLines=\"" << sylinderNumber << "\">\n";
    // Points
    file << "<Points>\n";
