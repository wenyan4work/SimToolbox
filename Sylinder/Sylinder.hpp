...
class Sylinder {
  public:
    ...
    double posCurrent[3];  ///< position at the beginning of the timestep
    double orientation[4];        ///< orientation quaternion
    double orientationCurrent[4]; ///< orientation quaternion at the beginning of the timestep
                                  ///< direction norm vector = orientation * (0,0,1)

    // vel = velBrown + velCon + velNonB
    // force =          forceCon + forceNonB
    // there is no Brownian force

    // velocity
    double vel[3];       ///< velocity = velCon+velNonB+velBrown
    double omega[3];     ///< angular velocity = omegaCon+omegaNonB+velBrown
    ...
    // force
    double force[3];      ///< force = forceCon+forceNonB
    double torque[3];     ///< torque = torqueCon+torqueNonB
    ...
    /**
     * @brief reset the final position and orientation to the position and orientation stored at the beginning of the timestep
     *
     */
    void resetConfiguration();

    /**
     * @brief advance the current position and orientation to the final position and orientation at the end of the timestep
     *
     */
    void advance();
...
#pragma omp parallel for
        for (int i = 0; i < sylinderNumber; i++) {
            ...
            // point and point data
            // use the position and orientation from the beginning of the timestep
