...
public:
  ...
  std::vector<DATA_TYPE>
      dataOnLocal; ///< data associated with ID on local mpi rank
  ...
  ZDD(GID_TYPE nEst) {
    ...
    error = this->findZDD.Create(MPI_COMM_WORLD, 1, 0,
                                 sizeof(DATA_TYPE) / sizeof(char), 0, 9);
    findZDD.Print();
    findZDD.Stats();
#else
    // normal mode, debug verbose level 0
    error = this->findZDD.Create(MPI_COMM_WORLD, 1, 0,
                                 sizeof(DATA_TYPE) / sizeof(char), 0, 0);
#endif
    if (error != ZOLTAN_OK) {
  ...
  int buildIndex() {
    ...
    int error =
        findZDD.Update(idPtr, NULL, (char *)dataPtr, NULL, gidOnLocal.size());
    if (error != ZOLTAN_OK) {
  ...
  int find() {
    ...
    // count, int *owner
    int status = findZDD.Find(idPtr, NULL, (char *)dataPtr, NULL,
                              gidToFind.size(), NULL);
    return status;
  }
};
