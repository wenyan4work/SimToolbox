    ...
    { // Determine bounding box: BBox.X, BBox.L
        ...
        {     // determine local bounding box
            { // Init X0, X1 <-- Broadcast one source or target point to all processors
                ...
                comm_.Allreduce<sctl::Integer>(sctl::Ptr2ConstItr<sctl::Integer>(&src_proc_, 1),
                                               sctl::Ptr2Itr<sctl::Integer>(&src_proc, 1), 1, sctl::Comm::CommOp::MAX);
            ...
            auto determine_bbox = [&](const sctl::Vector<Real> &coord, sctl::Iterator<Real> X0,
                                      sctl::Iterator<Real> X1) {
...
#pragma omp parallel for reduction(max : max_rad)
        for (sctl::Long i = 0; i < SRad.Dim(); i++)
            max_rad = std::max(max_rad, SRad[i]);
#pragma omp parallel for reduction(max : max_rad)
        for (sctl::Long i = 0; i < TRad.Dim(); i++)
            max_rad = std::max(max_rad, TRad[i]);
        comm_.Allreduce(sctl::Ptr2ConstItr<Real>(&max_rad, 1), sctl::Ptr2Itr<Real>(&max_rad_glb, 1), 1,
                        sctl::Comm::CommOp::MAX);
    ...
    { // Set period_length0 <-- period_length / BBox.L
        ...
        for (sctl::Integer i = 0; i < DIM; i++) {
            ...
            if (period_length[i] > 0)
                SCTL_ASSERT(period_length0[i] > 0);
        }
    }
    ...
    for (sctl::Integer d = 0; d < DIM; d++) { // Set c
        ...
        for (sctl::Integer k = 0; k < 3; k++) {
            ...
            if (c0_new >= 0 && c0_new < 1) {
                for (sctl::Integer i = 0; i < len * DIM; i++)
                    c[(new_len)*DIM + i] = c[i];
                for (sctl::Integer i = 0; i < len; i++)
                    c[(new_len + i) * DIM + d] = c0_new;
                new_len += len;
            }
        }
        len = new_len;
...
template <class Real, int DIM>
template <class SrcObj, class TrgObj>
void NearInteraction<Real, DIM>::SetupNearInterac(const std::vector<SrcObj> &src_vec,
                                                  const std::vector<TrgObj> &trg_vec) {
    ...
    {                              // sctl::Communicate ghost source data
        ...
        { // Set mins
            ...
            if (!rank)
                m = MID().DFD(depth);
            if (TData_.Dim())
                m = std::min(m, TData_[0].mid);
            if (SData_.Dim())
                m = std::min(m, SData_[0].mid);
...
#pragma omp parallel
    {
        ...
        for (sctl::Long t0 = lb; t0 < ub;) {
            ...
            while (t1 < ub && TData_[t1].mid == Tmid)
                t1++;
            ...
            for (sctl::Integer j = 0; j < nbrlst.Dim(); j++) {
                ...
                if (s0 < SData__.Dim() && SData__[s0].mid == src_key.mid) {
                    while (s1 < SData__.Dim() && SData__[s1].mid == src_key.mid)
                        s1++;
                }
...
template <class Real, int DIM>
template <class ObjType>
void NearInteraction<Real, DIM>::ForwardScatter(const std::vector<ObjType> &in_vec, std::vector<ObjType> &out_vec,
    ...
    { // Set rcnt0, rdsp0, scnt0, sdsp0
        for (sctl::Integer i = 0; i < np; i++) {
            Long idx0 = (i + 0 < np ? std::lower_bound(recv_idx.begin(), recv_idx.end(), mins[i + 0]) - recv_idx.begin()
                                    : recv_idx.Dim());
            Long idx1 = (i + 1 < np ? std::lower_bound(recv_idx.begin(), recv_idx.end(), mins[i + 1]) - recv_idx.begin()
                                    : recv_idx.Dim());
            rcnt0[i] = idx1 - idx0;
            rdsp0[i] = idx0;
        }
    ...
    comm_.Alltoallv<Long>(recv_idx.begin(), rcnt0.begin(), rdsp0.begin(), send_idx.begin(), scnt0.begin(),
                          sdsp0.begin());
    ...
    { // Init send_buff, ssize, rsize
        ...
        for (sctl::Long i = 0; i < send_idx.Dim(); i++) { // Set send_buff, ssize
            ...
            for (sctl::Long j = 0; j < tmp_buff.size(); j++)
                send_buff.PushBack(tmp_buff[j]);
            ssize[i] = tmp_buff.size();
        }
        { // Set rsize
            comm_.Alltoallv<Long>(ssize.begin(), scnt0.begin(), sdsp0.begin(), rsize.begin(), rcnt0.begin(),
                                  rdsp0.begin());
        }
    }
    ...
    { // out_vec <-- Unpack(recv_buff)
        ...
        for (sctl::Long i = 0; i < Nelem; i++) { // Unpack recv_buff
            ...
            for (sctl::Long j = 0; j < Nsize; j++)
                tmp_buff[j] = recv_buff[rdisp[i] + j];
...
template <class Real, int DIM>
template <class ObjType>
void NearInteraction<Real, DIM>::ReverseScatter(const std::vector<ObjType> &in_vec, std::vector<ObjType> &out_vec,
    ...
    { // Set scnt0, sdsp0, rcnt0, rdsp0
        for (sctl::Integer i = 0; i < np; i++) {
            Long idx0 = (i + 0 < np ? std::lower_bound(send_idx.begin(), send_idx.end(), mins[i + 0]) - send_idx.begin()
                                    : send_idx.Dim());
            Long idx1 = (i + 1 < np ? std::lower_bound(send_idx.begin(), send_idx.end(), mins[i + 1]) - send_idx.begin()
                                    : send_idx.Dim());
            scnt0[i] = idx1 - idx0;
            sdsp0[i] = idx0;
        }
    ...
    comm_.Alltoallv<Long>(send_idx.begin(), scnt0.begin(), sdsp0.begin(), recv_idx.begin(), rcnt0.begin(),
                          rdsp0.begin());
