...
#pragma omp section
   ...
   */
  template <class Data>
  void exchangeAllToAllV(std::vector<int> &sendDestRank,
...
template <class Data>
void CommMPI::exchangeAllToAllV(std::vector<int> &sendDestRank,
                                std::vector<Data> &sendData, //
                                std::vector<int> &recvSrcRank,
                                std::vector<Data> &recvData) {
...
#pragma omp section
  ...
  MPI_Alltoallv(sendData.data(), sendCounts.data(), sendDispls.data(),
                mpiDataType, //
                recvData.data(), recvCounts.data(), recvDispls.data(),
